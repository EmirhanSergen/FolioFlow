...
class Analytics
{
    ...
    /**
     * Fetch multiple trading metrics for the user.
     *
     * @param int $userId
     * @return array
     * @throws Exception
    ...
    public function getTradeMetrics($userId)
    {
        try {
            ...
            // Monthly profits (for best/worst month)
            ...
            $profitFactor   = ($stats['total_losses'] ?? 0) > 0
                ? ($stats['total_gains'] / $stats['total_losses'])
                : 0;
                ...
                SELECT 
                    ...
                    AVG(
                      CASE 
                          ...
                          WHEN s.current_price IS NOT NULL THEN (s.current_price - buy_price) * amount
                          ELSE 0 
                      END
                    ) as avg_profit
                FROM investments i
